 ...
 */
export const loadMenuOptions = async (selectedPost, user, isAuthenticated, token) => {
  if (!selectedPost) {
  ...
  const postUserId = selectedPost.user || selectedPost.userId || selectedPost.author;
  ...
  try {
    ...
    console.log('Filtered options:', filtered.map(o => o.text));
...
export const handleMenuOption = async (
  ...
  // Handle different user ID field names
  const userId = selectedPost.user || selectedPost.userId || selectedPost.author;
  
  if (!userId && ['Follow', 'Unfollow', 'Block', 'About this account'].includes(option.text)) {
  ...
  console.log('Handling menu option:', option.text, 'for post:', selectedPost.id || selectedPost._id);
  ...
  switch (option.text) {
    ...
    case 'Delete Post':
      success = await handleDeletePost(selectedPost.id || selectedPost._id, token, isAuthenticated);
...
export const usePostInteractions = (
  ...
  const handleMenuOptionPress = useCallback(async (option, selectedPost, setModalVisible) => {
    ...
    const callbacks = {
      ...
      onBlockSuccess: (userId) => {
        ...
        if (updatePosts) {
          updatePosts(posts => posts.filter(post => {
            const postUserId = post.userId || post.user || post.author;
            return postUserId !== userId;
          }));
        }
  ...
  const loadPostMenuOptions = useCallback(async (selectedPost) => {
    console.log('Loading menu options for post:', selectedPost);
    
    const result = await loadMenuOptions(selectedPost, user, isAuthenticated, token);
