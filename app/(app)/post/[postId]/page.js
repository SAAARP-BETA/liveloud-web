...
import { Loader2, UserPlus, UserMinus, Info, Flag, Ban, Trash2 } from 'lucide-react';
    ...
    return (
        <div className="flex items-start space-x-3 p-4 border-t border-gray-100 overflow-hidden">
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-gray-800 mt-1 break-words overflow-wrap-anywhere whitespace-pre-wrap">{comment.content}</p>
            </div>
        </div>
    );
});
...
const PostPage = () => {
    ...
    const handleCommentSubmit = useCallback(async (e) => {
        ...
        try {
            const response = await fetch(`${API_ENDPOINTS.SOCIAL}/posts/${postId}/comment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
    ...
    useEffect(() => {
        ...
        const loadOptions = async () => {
            ...
            try {
                ...
                // Fallback options
                const isOwnPost = isAuthenticated && user && (selectedPost.user === user._id || selectedPost.userId === user._id);
    ...
    if (error) return <div className="w-xl text-center py-20 text-red-500">Error: {error}</div>;
    if (!post) return <div className="w-xl text-center py-20">Post not found.</div>;
            ...
            <div className="container mx-auto w-xl py-4">
                ...
                <div className="bg-white rounded-xl mt-4">
                    ...
                    {post.comments && post.comments.length > 0 ? (
                        ...
                        <p className="px-4 pb-4 text-gray-500">No comments yet. Be the first to comment!</p>
                    )}
                </div>
            </div>
